...
exports.handler = (event, context, callback) => {
    Promise.all(event.records.map(r => {
        ...
        if (data.messageType === 'CONTROL_MESSAGE') {
            ...
            console.log(data);
            const promises = data.logEvents.map(logEvent => {
                console.log(logEvent);
                // tranform to elasticsearch start
                var payload = buildSource(logEvent.message, logEvent.extractedFields);
                payload['@id'] = logEvent.id;
                payload['@timestamp'] = new Date(1 * logEvent.timestamp).toISOString();
                payload['@message'] = logEvent.message;
                payload['@owner'] = data.owner;
                payload['@log_group'] = data.logGroup;
                payload['@log_stream'] = data.logStream;

                const out = JSON.stringify(payload);
                // tranform to elasticsearch
